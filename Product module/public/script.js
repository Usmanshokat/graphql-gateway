...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    const mutation = isEdit
      ? `
        mutation UpdateProduct($idValue: ID!, $nameValue: String!, $descriptionValue: String!) {
          updateProduct(
            id: $idValue
            name: $nameValue
      ...
      `
      : `
        mutation CreateProduct($nameValue: String!, $descriptionValue: String!) {
          createProduct(
            name: $nameValue
            description: $descriptionValue
          ) {
...
async function fetchItems() {
  ...
  try {
    ...
    itemsCache = result.data.products;    
...
tableBody.addEventListener('click', async (e) => {
  ...
  if (e.target.classList.contains('btn-delete')) {
    const confirmed = confirm('Are you sure you want to delete this prdouct?');
    if (!confirmed) return;

    const mutation = `
      mutation DeleteProduct($idValue: ID!) {
        deleteProduct(id: $idValue)
      }
    `;
